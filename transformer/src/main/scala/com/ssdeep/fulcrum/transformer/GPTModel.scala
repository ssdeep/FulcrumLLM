...
case class GPTModel(cfg: GPTConfig) extends torch.nn.Module with ClassNameLogger:
  private val tokEmbed = register(torch.nn.Embedding(cfg.vocabSize.intValue(), cfg.embedDim), "tokEmbed")
  protected[fulcrum] val posEmbed = register(torch.nn.Embedding(cfg.contextLength, cfg.embedDim), "posEmbed")
  ...
  lazy val totalParams: Long = this.parameters(true).map(_.numel).sum
  def printSizeInMB: String = {
    val totalSize = (totalParams*4.0)/(1024.0*1024)
    f"Total Model Size: $totalSize%.2f MB"
  }

  private val posEmbedsMap: scala.collection.mutable.Map[Int, Tensor[Float32]] = scala.collection.mutable.Map()
  logger.info(
    s"""
       |Initializing GPT Model With Parameters
       |Tokenizing Embeddings: ${tokEmbed}""".stripMargin)

  def apply(inIdx: Tensor[Int64]): Tensor[Float32] =
    val Seq(batchSize, seqLen) = inIdx.shape

    val tokEmbeds = tokEmbed(inIdx)
    val posEmbeds = posEmbed(torch.arange(end = seqLen, device = inIdx.device, dtype = torch.int64))
